...
export async function seed() {
  ...
  const path = './dev.db';
  const filebuffer: Buffer | null = fs.existsSync(path) ? fs.readFileSync(path) : null;
  ...
  // Insert or update merchant by email (idempotent)
  ...
  const merchantId = mRes[0].values[0][0] as string;
