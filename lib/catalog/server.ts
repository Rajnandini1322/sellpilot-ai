...
export async function getProducts({
    ...
    if (queryWords.length > 0) {
      const wordConditions = queryWords.map(
        (word) =>
          ...
          `lower(description) LIKE '%${word}%' OR ` +
          `lower(category) LIKE '%${word}%' OR ` +
          `lower(tags) LIKE '%${word}%')`
      );

      where.push(`(${wordConditions.join(" OR ")})`);
    }
  }
  ...
  const countRes = db.exec(
    `SELECT COUNT(*) as cnt FROM Product ${whereSql};`
  );
  ...
  // This is important for AI search queries such as:
  // "Ergo Travel Mouse"
...
export async function getAgentCatalog() {
  ...
  const mres = db.exec(
    `SELECT id, name FROM Merchant LIMIT 1;`
  );
  ...
  const merchant =
    ...
    mres[0].values[0]
      ? {
          id: mres[0].values[0][0],
          name: mres[0].values[0][1],
        }
      : {
          id: "unknown",
          name: "Unknown",
